...
def message_routing(original_message):
    ...
    if message.casefold() == "quit":
        ...
        router_in.send(bytes(router_mac + router_in_ip + source_ip + source_mac + "OUT", "utf-8"))
        return

    else:
        print(str(datetime.now()) + "\tMessage sent by: " + source_ip + ":\t\t" + message)
    ...
    elif destination_ip[0:10] == router_out_ip[0:10] and not source_ip[0:10] == router_in_ip[0:10]:
        router_in.send(bytes(router_mac + router_out_ip + source_ip + source_mac + "L'utente " +
                             destination_ip + " non è online al momento", "utf-8"))

    # if the client isn't internal to the current router net, the message will be sent to the server
    else:
        router_in.send(bytes(router_mac + source_ip + destination_ip + source_mac + message, "utf-8"))
...
def receive_message_server():
    while True:
        message = router_in.recv(2048).decode("utf-8")
        message_routing(message)
...
if __name__ == '__main__':
    ...
    thread_server = Thread(target=receive_message_server)
